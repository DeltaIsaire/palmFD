...
#   output/FD_tdwg3_trait_means_gapfilled.csv
#   output/FD_tdwg3_trait_means_unfilled.csv
...
traits.gapfilled <- read.csv(file = "output/traits_filled_genus_mean.csv")
traits.unfilled <-
   read.csv(file = "output/palm_traits.csv") %>%
  .[complete.cases(.), ]
...
write.csv(matrix.gapfilled,
          file = "output/FD_palm_traits_transformed_gapfilled.csv",
          eol = "\r\n",
          row.names = TRUE
          )
...
write.csv(matrix.unfilled,
          file = "output/FD_palm_traits_transformed_unfilled.csv",
          eol = "\r\n",
          row.names = TRUE
          )
...
write.csv(pres.abs.gapfilled,
          file = "output/FD_palm_tdwg3_pres_abs_gapfilled.csv",
          eol = "\r\n",
          row.names = TRUE
          )
...
write.csv(pres.abs.unfilled,
          file = "output/FD_palm_tdwg3_pres_abs_unfilled.csv",
          eol = "\r\n",
          row.names = TRUE
          )
...
# "unfilled".
  ...
  write.csv(fd.indices,
            file = paste0("output/FD_indices_", name, ".csv"),
            eol = "\r\n",
            row.names = TRUE
            )
  ...
  write.csv(community.means,
            file = paste0("output/FD_tdwg3_trait_means_", name, ".csv"),
            eol = "\r\n",
            row.names = TRUE
            )
  return (0)
